...
export const verifyJWT = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const decoded = jwt.verify(token, process.env.ACCESS_TOKEN_SECRET as string) as checkUser;
    
    const user = await User.findById(decoded._id).select("-password -refreshToken") as checkUser;
