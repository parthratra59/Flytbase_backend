    ...
    if (!drone_id || !drone_type || !make_name || !name || !siteID) {
      res
        ...
        .json(
          new ApiError(
            400,
            "All fields are required and drone_id,drone_type,make_name,name should be unique"
          )
        );
    }

    // Check if a drone with the same unique fields already exists
    const existingDrone = await Drone.findOne({ drone_id });
...
export const DeleteDroneById = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const deletedDrone = await Drone.findByIdAndDelete(_id, {
      $set: {
